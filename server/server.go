...
import (
	...
	examService "github.com/thekure/examDisys/proto"
...
func main() {
	...
	examService.RegisterServiceServer(grpcServer, server)
...
func (s *Server) Bid(ctx context.Context, RequestBid *examService.RequestBid) (*examService.Ack, error) {
...
func (s *Server) Result(ctx context.Context, RequestBid *examService.HighestBidRequest) (*examService.Outcome, error) {
	log.Printf(RequestBid.Message)
	currentHighestBid := strconv.Itoa(int(s.currentHighestBid))

	if !s.isAuctionRunning {
		return &examService.Outcome{
			Amount: s.currentHighestBid,
